...
The fix changes `TwoPhaseWrite.execute()`'s semantics from refuse-now to wait-until-decided. `execute()` consumes first; on an `AWAITING_APPROVAL` refusal it waits — polling the core's public `listPending()` on a 100ms interval, bounded by the plan TTL plus a small grace period so an unchosen plan can never hold the call open forever — then re-consumes. The core's `consume()` deliberately does not mark a token used on an `AWAITING_APPROVAL` refusal, so the re-consume is safe: an approval lets the statement execute, a rejection returns `PLAN_REJECTED` with the human's reason, and an expiry returns `EXPIRED_TOKEN`.
